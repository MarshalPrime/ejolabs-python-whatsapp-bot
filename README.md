# Build AI WhatsApp Bots with Pure Python (EjoLabs)

This guide will walk you through the process of creating a WhatsApp bot using the Meta (formerly Facebook) Cloud API with pure Python, and Flask in particular. We'll also integrate webhook events to receive messages in real-time and use EjoChat to generate AI responses. For more information on the structure of the Flask application, you can refer to [this documentation](https://github.com/MarshalPrime/ejolabs-python-whatsapp-bot/tree/main/app).
...
- [Build AI WhatsApp Bots with Pure Python](#build-ai-whatsapp-bots-with-pure-python)
  ...
  - [EjoLabs & EjoChat](#ejolabs--ejochat)
...
3. Let's convert that into a [Python function with the request library](https://github.com/MarshalPrime/ejolabs-python-whatsapp-bot/blob/main/start/whatsapp_quickstart.py).
4. Create a `.env` file based on `example.env` and update the required variables.
...
2. On the System Users page, configure the assets for your System User, assigning your WhatsApp app with full control. Don't forget to click the Save Changes button.
   - [See step 1 here](https://github.com/MarshalPrime/ejolabs-python-whatsapp-bot/blob/main/img/meta-business-system-user-token.png)
   - [See step 2 here](https://github.com/MarshalPrime/ejolabs-python-whatsapp-bot/blob/main/img/adding-assets-to-system-user.png)
...
- Run your Flask app locally by executing [run.py](https://github.com/MarshalPrime/ejolabs-python-whatsapp-bot/blob/main/run.py)
...
## Step 6: Integrate AI into the Application (EjoChat)

Now that we have an end-to-end connection, we can make the bot intelligent by routing incoming messages to EjoChat.

1. Get your API Key from [EjoLabs](https://ejolabs.com/api).
2. Add your `EJOCHAT_API_KEY` and `EJOCHAT_API_URL` to your `.env` configuration file.
3. The server will use [ejochat_service.py](https://github.com/MarshalPrime/ejolabs-python-whatsapp-bot/blob/main/app/services/ejochat_service.py) to send requests to EjoChat.
4. When a user sends a WhatsApp message, it triggers `process_whatsapp_message()` in [whatsapp_utils.py](https://github.com/MarshalPrime/ejolabs-python-whatsapp-bot/blob/main/app/utils/whatsapp_utils.py), which queries EjoChat and replies to the sender automatically.
...
This repository is maintained and customized for EjoLabs use cases. We build custom conversational AI solutions powered by EjoChat to help automate user engagement.

* **Website**: [ejolabs.com](https://ejolabs.com)
* **API Documentation**: [ejolabs.com/api](https://ejolabs.com/api)
* **Product**: [EjoChat](https://chat.ejolabs.com/) is an advanced conversational model designed for natural interactions in local languages (such as Kinyarwanda) and English.
